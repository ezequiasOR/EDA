...
class BuscaBST {
	public static void main(String[] args) {
		...
		int[] elements = getArrayInteiros(sc);
...
class BSTBusca<T extends Comparable<T>> {
	...
	public BSTBusca() { root = new BSTNodeBusca<T>(); }

	public BSTNodeBusca<T> getRoot() { return this.root; }

	public boolean isEmpty() { return root.isEmpty(); }

	public void insert(T element) {
		if (element != null) { this.insert(root, element); }
	}
	...
	public void insert(BSTNodeBusca<T> node, T element) {
		if (node.isEmpty()) {
			...
			if (element.compareTo(node.getData()) > 0) { insert(node.getRight(), element); }
	...
	private void search(BSTNodeBusca<T> node, T element, List<T> lista) {
		if (!node.isEmpty()) {
			...
			else if (element.compareTo(node.getData()) > 0) { this.search(node.getRight(), element, lista); }
			else { this.search(node.getLeft(), element, lista); }
		}
	}
}
...
class BSTNodeBusca<T> {
	...
	public BSTNodeBusca(T value, BSTNodeBusca<T> left, BSTNodeBusca<T> right, BSTNodeBusca<T> parent) {
		super();
		this.value = value;
		this.left = left;
		this.right = right;
	...
	public BSTNodeBusca<T> getLeft() { return left; }
	
	public void setLeft(BSTNodeBusca<T> left) { this.left = left; }
	
	public BSTNodeBusca<T> getRight() { return right; }
	
	public void setRight(BSTNodeBusca<T> right) { this.right = right; }
	
	public BSTNodeBusca<T> getParent() { return parent; }
	
	public void setParent(BSTNodeBusca<T> parent) { this.parent = parent; }
