...
class BSt<T extends Comparable<T>> {
	...
	public T[] preOrder() {
		...
		if (!isEmpty()) { this.preOrder(root, lista); }
	...
	public void balancear() {
		if (estaBalanceada()) { System.out.println("balanceada"); }
		else { rebalance(root); }
	}

	public boolean estaBalanceada() { return estaBalanceada(root); }

	private boolean estaBalanceada(BStNode<T> node) {
		boolean balanceado = false;

		if (node.isLeaf() || node.isEmpty()) { balanceado = true; }
		else {
			if (nodeBalanceado(node)) { balanceado = true; }
